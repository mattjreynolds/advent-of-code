...
def get_weight(prg):
    weight = int(prg['weight'])
    child_weights = [get_weight(p) for p in prg['children'].values()]

    if len(set(child_weights)) > 1:
        counts = Counter(child_weights)
        for w, v in counts.items():
            if v == 1:
                wrong_weight = w
            else:
                right_weight = w
        weight_diff = right_weight - wrong_weight

        for p in prg['children'].values():
            if get_weight(p) == wrong_weight:
                p['new_weight'] = p['weight'] + weight_diff
                child_weights = [right_weight] * len(child_weights)
...
def day_07(inp=inp):
    lines = [i.strip() for i in inp.splitlines()]

    prgs = defaultdict(dict)
    for l in lines:
        prg, chil = (
            l.split(' -> ') 
            if ' -> ' in l 
        ...
        p,w = prg.split(' ')
        children = chil.split(', ') if chil else []

        prgs[p]['name'] = p
        prgs[p]['weight'] = int(w.strip('()'))
        prgs[p]['child_names'] = children
    ...
    for name, prg in prgs.items():
        if 'parent' not in prg:
            base = name

    for prg in prgs.values():
        prg['children'] = {c: prgs[c] for c in prg['child_names']}
    
    weight = get_weight(prgs[base])
    new_weight = [p['new_weight'] for p in prgs.values() if 'new_weight' in p][0]
...
def day_07_part_2(inp=inp):
    ret = day_07(inp=inp)
    ret['result'] = ret['new_weight']
    return ret
