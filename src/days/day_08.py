...
def day_08(inp=inp):
    ...
    for l in lines:
        reg, op, val, if_, if_reg, if_test, if_val = l.split(' ')
        cmds.append({
            'reg': reg,
            'op' : ops[op],
            'val': int(val),
    ...
    for c in cmds:
        if c['if']['test'](registers[c['if']['reg']], c['if']['val']):
            registers[c['reg']] = c['op'](registers[c['reg']], c['val'])
...
def day_08_part_2(inp=inp):
    ret = day_08(inp=inp)
    ret['result'] = ret['highest']
    return ret
