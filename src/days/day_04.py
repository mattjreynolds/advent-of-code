inp = '''vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
def row_valid(row, allow_anagrams=True):
    return (len(row.split()) == len(set(
        [(r if allow_anagrams else ''.join(sorted(r))) for r in row.split()]
    )))


def day_04(inp=inp, allow_anagrams=True):
    rows = inp.splitlines()

    valid = [row for row in rows if row_valid(row, allow_anagrams)]
...
def day_04_part_2(inp=inp):
    return day_04(inp=inp, allow_anagrams=False)
